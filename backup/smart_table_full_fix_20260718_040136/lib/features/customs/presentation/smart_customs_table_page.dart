...
class SmartCustomsTablePageState extends State<SmartCustomsTablePage> {
  ...
  _RecordStatus _statusOf(CustomsRecord record) {
    ...
    if (record.customsAmount <= tolerance) {
    ...
    if (record.paidAmount + tolerance < record.customsAmount) {
      return _RecordStatus.partial;
    }
    if ((record.paidAmount - record.customsAmount).abs() <= tolerance) {
  ...
  Future<void> _moveSelectedRowToTypedNumber() async {
    if (_sort != _TableSort.manual) {
      _message('لتغيير ترتيب الصفوف اختر الترتيب اليدوي أولاً');
      return;
    }

    if (_searchController.text.trim().isNotEmpty ||
        _filter != _TableFilter.all) {
      _message('إعادة ترتيب الصفوف متاحة عند عرض القائمة بدون بحث أو فلتر');
      return;
    }
  ...
  _AccountSummary _accountSummary({
    ...
    final totalCustoms = records.fold(
      0.0,
      (sum, record) => sum + record.customsAmount,
    );
  ...
  String _accountStatementMessage(_AccountSummary summary) {
    ...
    final grandTotal = rows.fold(
      0.0,
      (sum, row) => sum + _recordMessageGrandTotal(row.record),
    );
    final lines = <String>[
      'الأخ/ ${summary.name}',
    ...
    for (final row in rows) {
      ...
      final radiologyFee = _recordMessageRadiologyFee(record);
      final customsAndClearanceAmount =
          _recordMessageCustomsAndClearanceAmount(record);
      lines.add(
  ...
  double _recordMessageRadiologyFee(CustomsRecord record) {
    if (!record.radiologyFeeApplied) return 0;
    return 10000;
  }

  double _recordMessageCustomsAndClearanceAmount(CustomsRecord record) {
    final radiologyFee = _recordMessageRadiologyFee(record);
    final customsAmountWithoutRadiology = (record.customsAmount - radiologyFee)
        .clamp(0.0, double.infinity)
        .toDouble();
    return customsAmountWithoutRadiology + record.clearanceFee;
  }

  double _recordMessageGrandTotal(CustomsRecord record) {
    return _recordMessageCustomsAndClearanceAmount(record) +
        _recordMessageRadiologyFee(record) +
        record.driverAdvance;
  }
  ...
  Future<void> _openCellCalculator({
    ...
    final initialValue = double.tryParse(displayValue.replaceAll(',', '')) ?? 0;
    final result = await showDialog<double>(
      context: context,
      builder: (context) => _CalculatorDialog(initialValue: initialValue),
    );
  ...
  SmartTableExportData _buildExportData({
    ...
    final totalCustoms = rows.fold(
      0.0,
      (sum, row) => sum + row.record.customsAmount,
    );
    ...
    final finalBalance = rows.fold(
      0.0,
      (sum, row) => sum + row.record.customsAmount - row.record.paidAmount,
    );

    final columns = _visibleColumns
        .map((column) => SmartTableExportColumn(
              id: column.name,
              label: column.label,
            ))
        .toList();
  ...
  _VisibleSummary _calculateVisibleSummary(List<_TableRowModel> rows) {
    ...
    for (final row in rows) {
      ...
      totalCustoms += record.customsAmount;
      ...
      if (record.paidAmount <= 0.01 && record.customsAmount > 0.01) {
        unpaidCount++;
      }
  ...
  Widget _buildSummaryPanel(
    ...
    return Wrap(
      ...
      children: [
        ...
        _buildSummaryCard(
          width: cardWidth,
          title: 'إجمالي الجمارك',
          value: _money(summary.totalCustoms),
          icon: Icons.account_balance_wallet_outlined,
          color: Colors.blue,
        ),
        _buildSummaryCard(
  ...
  Widget _tableViewport({
    ...
    return Expanded(
      child: rows.isEmpty
          ? SingleChildScrollView(child: _emptyRowsState(records))
          : _zoomableTable(rows),
    );
  }
  ...
  Widget _tableViewportBox({
    ...
    if (rows.isEmpty) {
      return _emptyRowsState(records);
    }

    return SizedBox(
      height: height,
      child: _zoomableTable(rows),
    );
  }
            ...
            cells: _visibleColumns.map((column) {
              switch (column) {
                case _TableColumn.date:
                  ...
                  return DataCell(
                    _sizedCell(column, _cellText(value)),
                ...
                case _TableColumn.unitPrice:
                  return _editableDataCell(
                    record,
                    column,
                    record.unitPrice == null ? '' : _number(record.unitPrice!),
                ...
                case _TableColumn.merchant:
                  ...
                  return _editableDataCell(
                    ...
                    column,
                    merchantName ?? '',
                    placeholder: merchantName == null || merchantName.isEmpty
                        ? '-'
                        : merchantName,
                    onTap: merchantName == null || merchantName.isEmpty
                        ? null
                        : () => _openMerchant(record),
                    isLink: merchantName != null && merchantName.isNotEmpty,
                  );
                case _TableColumn.paidAmount:
                  ...
                  return DataCell(
                    _sizedCell(
                      ...
                      _numberText(
                        value,
                        color:
                            row.runningBalance < 0 ? Colors.green : Colors.red,
                        bold: true,
                      ),
                    ),
                    onDoubleTap: () => _showCellActionSheet(
                ...
                case _TableColumn.status:
                  return DataCell(
                    _sizedCell(column, _StatusChip(status: row.status)),
                    onDoubleTap: () => _showCellActionSheet(
                      record: record,
                      fieldKey: column.name,
                ...
                case _TableColumn.actions:
                  return DataCell(_sizedCell(column, _actions(record)));
              }
            }).toList(),
          );
        }).toList(),
  ...
  DataCell _editableDataCell(
    ...
    if (_editingRecord?.id == record.id && _editingColumn == column) {
      return DataCell(
        _sizedCell(column, _inlineEditor(record, column, numeric: numeric)),
      );
    }
